...
public class GetAllPartiesServlet extends HttpServlet {
    
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        RequestDispatcher rqd=null;        
        try{
              Map<String,String> parties=CandidateDAO.getAllParties();
              request.setAttribute("parties", parties);
              rqd=request.getRequestDispatcher("responseallparties.jsp");
        }
        catch(Exception ex)
        {
            ex.printStackTrace();           
