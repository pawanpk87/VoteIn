...
public class ElectionResultControllerServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            Map<CandidateDetails,Integer> resultDetails=new LinkedHashMap<>();
            for(Map.Entry<String,Integer> data : result.entrySet())
            {
                String candidateId=data.getKey();
                int totalVotes=data.getValue();
                CandidateDetails candidate=CandidateDAO.getDetailsById(candidateId);
                resultDetails.put(candidate, totalVotes);
            }          
            request.setAttribute("votecount", VoteDAO.getVoteCount());
            request.setAttribute("result", resultDetails);
            rqd=request.getRequestDispatcher("electionresult.jsp");
        }
        catch(Exception ex)
        {
            ex.printStackTrace();           
