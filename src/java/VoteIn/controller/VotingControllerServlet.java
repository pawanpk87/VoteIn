...
public class VotingControllerServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(candidateId == null)
            {
                ArrayList<CandidateInfo> candidateList=CandidateDAO.viewCandidates(userid);
                request.setAttribute("candidatelist", candidateList);
                rqd=request.getRequestDispatcher("showcandidate.jsp");
            }
            else
            {
                CandidateInfo cinfo=VoteDAO.getVote(candidateId);
                request.setAttribute("candidate", cinfo);
                rqd=request.getRequestDispatcher("votedenied.jsp");
            }
        }
        catch(Exception ex)
        {
